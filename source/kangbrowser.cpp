...
void KangBrowser::completeLine(QString &line, int idx, bool bP, const QString &rstr) {
  //
  // I've modified the colours below to use a darker green that is more viewable
  ...
  if (bP) line.prepend("<font color=#007700>"); 
  if ((idx&2) || (idx&1) || bP) {
    line.append("</font>");
    line.replace( QRegExp("\\[D\\]"), "<font color=#000000>[D]</font>");
    line.replace( QRegExp("\\[E\\]"), "<font color=#000000>[E]</font>");
  } else {
    line.replace( QRegExp("\\[D\\]"), "<font color=#007700>[D]</font>");
    line.replace( QRegExp("\\[E\\]"), "<font color=#007700>[E]</font>");
  }
